...
let replaceTemplate=(temp,product)=>{
    ...
    if(!product.orgainc) output =output.replace(/{%NOT_ORGANIC%}/g,'not-organic');

    return output;        


    // Dont maipulate the args in the function
...
let server=http.createServer((req, res) =>{
    ...
    if (pathName==='/'  || pathName==='/overview'){
        ...
        let cardsHTML=dataObj.map(ele=>replaceTemplate(tempCard,ele)).join(' ');
